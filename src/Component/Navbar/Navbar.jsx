...
export default function Navbar({ userData, logout }) {
  const favorites = useSelector((state) => state.favorites.games);
  return <>
    <nav className="navbar navbar-expand-lg navbar-dark bg-dark fixed-top top-0">
      <div className="container">
        <Link className="navbar-brand" to=''><img src="./images/logo.png" className='navbar-brand' alt="" />GameWorld</Link>
        ...
        <div className="collapse navbar-collapse ms-4" id="navbarSupportedContent">
          ...
          <ul className="navbar-nav mb-2 ms-auto mb-lg-0">
            <ul className='navbar-nav mb-2 ms-auto mb-lg-0 align-items-center'>
              {userData ? 
              <>
              <li className='px-2 fs-'><Link to='/favorites'>Favorites <span  className='badge badge bg-warning pointer'>{favorites.length}</span></Link></li>
              <li className='px-2'><span onClick={logout} className='btn btn-outline-primary pointer'>Logout</span></li>
              </> 
              : <>
